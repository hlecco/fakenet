...
fn aplicacao_transmissora() {
    ...
    io::stdin().read_line(&mut message).expect("Leitura do STDIN falhou");
...
fn camada_de_enlace_transmissora(bytes: &[u8]) {
    ...
    println!("Verificação de erro: stream {:?} se torna {:?}", bits, bits_extended);
...
fn meio_de_comunicacao(bits: &[u8]) {
    let bits_transmitted: Vec<u8> = bits.iter()
        .map(|bit| bit * ((consts::ERRO < random()) as u8))
        .collect();
    println!("Erros no meio de transmissão: {}%", consts::ERRO*100.0);
    println!("Stream original: {:?}\nStream com erros: {:?}", bits, bits_transmitted);
...
fn camada_de_enlace_receptora(bits: &[u8]) {
    let original_message = match consts::CONTROLE {
        ...
        _ => Err(String::from("Apenas valores de 0 a 2 são válidos para controle de erros"))
    };

    let original_message = original_message.unwrap();
    println!("Recebido pela camada de enlace do receptor: {:?}\nInterpretado: {:?}", bits, original_message);
